...
for h in n_hidden:                          #looking or the best parameters w.r.t the number of hidden layers
    for n in n_neurons:                     #looking or the best parameters w.r.t the number of neurons
        for l in l_rate:                    #looking or the best parameters w.r.t the learning rate
            ...
            model = build_model(h, n, 10, l) # 10 output neurons since there are 10 classes
            ...
            for x, y in zip(X_test, y_test):
                x = (np.expand_dims(x,0)) # Need to add into a batch where it's the only member. Previous x.shape = (28, 28). New x.shape = (1, 28, 28). Model was trained on (nImages, 28, 28). Has to be the same dimensions
                
                pred = np.argmax(model.predict(x), axis=-1)
                correct_pred += 1 if pred == y else 0
                total_pred += 1
